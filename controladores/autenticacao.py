            ...
            while ok != "ok":
                ...
                with open('./dados/armazenamento_cadastros.txt', 'r') as arquivo:
                    ...
                    if (usuario.strip() == '') or (senha.strip() == ''):
                        limpar_tela()
                        print(
                            "Você precisa digitar um nome de usuário e uma senha.")
                        time.sleep(2)
                        continue
                    ...
                    if any(caracter in usuario for caracter in carcacteres_proibidos) or any(caracter in senha for caracter in carcacteres_proibidos):
                        limpar_tela()
                        print(
                            "A sua senha ou nome de usuário não pode conter espaços, barras ou o caractere '|'.")
                        time.sleep(2)
                        continue
                    ...
                    for usuarioCadastrado in usuarios:
                        arquivo_usuario = usuarioCadastrado.strip().split('|')[
                            0]
